...
Use `just serve` (adds headers automatically) or any server that
supports custom response headers.

## Quick Start

```bash
just build   # compile Mandelbrot to WebGPU
just serve   # start server at http://localhost:8000/standalone.html
```

The `standalone.html` page includes inline diagnostics:
- Checks WebGPU, SharedArrayBuffer, and Float16Array availability
- Reports compute/render timing
